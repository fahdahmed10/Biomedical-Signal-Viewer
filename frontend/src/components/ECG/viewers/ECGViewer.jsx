...
export default function ECGViewer({ data }) {
  ...
  // Clone signals and channels to state so we can modify (add combinations)
  const [signals, setSignals] = useState({ ...origSignals });
  const [channels, setChannels] = useState([...origChannels]);

  const [index, setIndex] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [speedOption, setSpeedOption] = useState(1);

  const [visibleChannels, setVisibleChannels] = useState([...origChannels]);
  const [colors, setColors] = useState(
    Object.fromEntries(origChannels.map((ch,i) => [ch, defaultColors[i % defaultColors.length]]))
  );
  ...
  useEffect(() => {
    ...
    setColors(Object.fromEntries(origChannels.map((ch,i)=>[ch, defaultColors[i % defaultColors.length]])));
    setIndex(0);
    setIsPlaying(false);
  }, [data]);
  ...
  useEffect(() => {
    if(isPlaying && time.length>0) requestRef.current = requestAnimationFrame(animate);
    else cancelAnimationFrame(requestRef.current);
    return () => cancelAnimationFrame(requestRef.current);
  }, [isPlaying, speedOption, time]);
  ...
  const createCombination = () => {
    ...
    let comboName = "";
    if (selectedForCombination.length === channels.length) {
    ...
    selectedForCombination.forEach(ch => {
      for(let i=0;i<time.length;i++){
        comboSignal[i] += signals[ch][i] || 0;
      }
    });

    setSignals(prev => ({ ...prev, [comboName]: comboSignal }));
    setChannels(prev => [...prev, comboName]);
  ...
  const traces = visibleChannels.map(ch => ({
    x: visibleTime,
    y: signals[ch]?.slice(index,index+windowSize) || [],
    type: "scatter",
    mode: "lines",
    name: ch,
      ...
      <div style={{ width: 220, borderRight: "1px solid #ccc", paddingRight: 10 }}>
        ...
        {channels.length > 0 && (
          <div style={{ marginBottom: 8 }}>
            <button onClick={() => setVisibleChannels([...channels])} style={{ marginRight: 5, padding:"4px 8px", cursor:"pointer" }}>Select All</button>
        ...
        {showCombinationUI && (
          <div style={{ border:"1px solid #ccc", padding:8, marginBottom:8, maxHeight:200, overflowY:"auto" }}>
            ...
            <button onClick={() => setSelectedForCombination([...channels])} style={{ marginRight:5 }}>Select All</button>
